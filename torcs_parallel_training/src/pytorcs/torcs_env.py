...
from pytorcs.torcs_client import TorcsClient, Sensor, TorcsState
from pytorcs.utils import get_orchestrator
...
class TorcsEnv(gym.Env):
    ...
    def __init__(self, reward_function: Callable[[TorcsState, TorcsState], float], sensor_features=None, truncate_limit=5000, port: int = 3001,
    ...
    def step(self, action: ActType):
        raw_steer = float(action[0]) ** 3

        self.client.actions.steering = raw_steer
        ...
        current_state = TorcsState(self.client.sensors, self.client.actions)
        ...
        self.previous_state = TorcsState(
            sensors=copy.copy(current_state.sensors),
            actions=copy.copy(current_state.actions)
        )
    ...
    def reset(self, seed: int=None, options: dict[str, Any]=None) -> np.ndarray:
        ...
        if not self.skip_reset_kill:
            ...
            orchestrate.kill_torcs_on(self.port)
        else:
