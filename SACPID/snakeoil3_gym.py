...
def _sacpid_scr_debug(hypothesis_id: str, location: str, message: str, data: dict) -> None:
    ...
    try:
        log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".cursor", "debug-bea265.log")
    ...
    def get_servers_input(self):
        ...
        while True:
            ...
            if '***identified***' in sockdata:
                ...
                _sacpid_scr_debug(
                    "H1",
                    "snakeoil3_gym.py:Client.get_servers_input:shutdown",
                    "SCR sent shutdown; closing socket",
                    {
                        "port": int(self.port),
                        "preview": sockdata[:240].replace("\n", " "),
                    },
                )
                # #endregion
                ...
                self.shutdown()
                return
            elif '***restart***' in sockdata and '(' not in sockdata:
                # #region agent log
                consecutive_restart_signals += 1
                _sacpid_scr_debug(
                    "H2",
                    "snakeoil3_gym.py:Client.get_servers_input:restart",
                    "SCR restart-only datagram; ack and recv again",
                    {
                        "port": int(self.port),
                        "consecutive_restart_signals": int(consecutive_restart_signals),
                        "preview": sockdata[:240].replace("\n", " "),
                    },
                )
                # #endregion
                _verb = os.environ.get("TORCS_VERBOSE_SCR_CONTROL", "").strip() in (
                    "1",
                    "true",
                    "yes",
                )
                if consecutive_restart_signals == 1 or _verb:
                    print(
                        "Server signaled restart on %d (control message); "
                        "acknowledging and waiting for telemetry." % self.port
                    )
                # Clear meta so we do not re-request SCR restart (ping-pong with gym_torcs meta sync).
                self.R.d['meta'] = 0
                try:
                    self.respond_to_server()
                except Exception:
                    pass
                if consecutive_restart_signals > 24:
                    print(
                        "SCR still sending restart-only on %d; reconnecting UDP client."
                        % self.port
                    )
                    self.shutdown()
                    self.setup_connection()
                ...
                cleaned = (
                    sockdata.replace("***restart***", "")
                    .replace("***identified***", "")
                    .strip()
                )
                self.S.parse_server_str(cleaned)
                consecutive_restart_signals = 0
                if self.debug:
...
class DriverAction():
    ...
    def clip_to_limits(self):
        ...
        if self.d['gear'] not in [-1, 0, 1, 2, 3, 4, 5, 6]:
            self.d['gear']= 0
        # gym_torcs uses Python bool True/False; only 0/1 are valid SCR meta values.
        if self.d['meta'] is True:
            self.d['meta'] = 1
        elif self.d['meta'] is False:
            self.d['meta'] = 0
        elif self.d['meta'] not in [0, 1]:
            self.d['meta'] = 0
